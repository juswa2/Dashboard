        ...
        <div className="flex flex-wrap -mx-3">
          <div className="profilebox11 w-full px-3 lg-max:mt-6 xl:w-4/12">
            <div className="relative flex flex-col min-w-0 break-words border-0 rounded-2xl bg-clip-border">
              ...
              {account.image ? (
                  <img src={`http://localhost:8081/uploads/${account.image}`} className="clientIcon fixed-size-image text-white text-2xl mr-4 cursor-pointer" alt="User Icon"/>
                ) : (
              ...
              <div className="namelabel">
                <h5 className="mb-0" style={{ fontWeight: 'bold', fontSize: '25px' }}>{`${account.first_name || ''} ${account.middle_name ? account.middle_name + ' ' : ''}${account.last_name || ''} ${account.suffix || ''}`}</h5>
              </div>
            </div>
          </div>
            ...
            <div className="relative flex flex-col h-full min-w-0 break-words border-0 shadow-soft-xl rounded-2xl bg-clip-border">
              ...
              <div className="flex-auto p-4">
                <ul className="flex flex-col pl-0 mb-3 mt-3 rounded-lg ml-4" style={{ fontSize: '20px', fontWeight: 'bold' }}>
                  ...
                  <li className="relative block px-4 py-4 pt-0 pl-0 leading-normal border-0 rounded-t-lg text-sm text-inherit" style={{ fontSize: '18px', fontWeight: '500' }}><strong className="text-white">Phone #:</strong> &nbsp; &nbsp;<span className="text-white font-medium text-sm" style={{ fontSize: '18px', fontWeight: '600' }}>{account.pnumber}</span></li>
                  <li className="relative block px-4 py-4 pt-0 pl-0 leading-normal border-0 rounded-t-lg text-sm text-inherit" style={{ fontSize: '18px', fontWeight: '500' }}><strong className="text-white">FB Account:</strong> &nbsp; &nbsp;<span className="text-white font-medium text-sm" style={{ fontSize: '18px', fontWeight: '600' }}>{account.fb}</span></li>
                  ...
                  <li className="relative block px-4 py-4 pt-0 pl-0 leading-normal border-0 rounded-t-lg text-sm text-inherit" style={{ fontSize: '18px', fontWeight: '500' }}><strong className="text-white">Password:</strong> &nbsp; &nbsp;<span type className="text-white font-medium text-sm" style={{ fontSize: '18px', fontWeight: '600' }}>{account.password.replace(/./g, '*')}</span></li>
                ...
                {/* Close button inside the client's information box */}
