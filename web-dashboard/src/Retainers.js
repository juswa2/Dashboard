    ...
    <div className="flex flex-col md:flex-row h-screen pagescreen">
      <div className="bg-gray-800 w-full md:w-64 navbar md:flex flex-col hidden">
        ...
        <nav className="mt-10">
          ...
          <Link to='/retainers' onClick={() => handleLinkClick('/retainers')} className={`buttonnav3 flex items-center py-5 px-4 ${activeLink === '/retainers' ? 'bg-white text-black' : 'hover:bg-gray-400 hover:text-black'}`}>
        ...
        <div className="mt-[12%] flex flex-wrap justify-start" style={{ display: isPhone ? 'grid': '', gridTemplateColumns: isPhone ? 'repeat(2, 1fr)' : 'auto', marginBottom: isPhone ? '50px' : ''}}>
          {filteredData.filter(accounts => accounts.account_type === 4).map((accounts, index) => (
            <Link to={`/retainerinfo/${accounts.id}`} key={index} className="clientLink">
              <div className="clientbox p-6 mr-1 mb-10 ml-20" style={{ width: isPhone ? '75%' : '300px', marginLeft: isPhone ? '13%' : '', height: isPhone ? '40%' : '', paddingBottom: isPhone ? '140px' : '', marginBottom: isPhone ? '-40px' : '40%'}}>
                <div className="mt-[-50%]">
                  {accounts.image ? (
                    <img src={`http://localhost:8081/uploads/${accounts.image}`} className="clientIcon fixed-size-image text-white text-2xl mr-4 cursor-pointer" alt="User Icon" style={{width: isPhone ? '100%' : '100%', height: isPhone ? '120px' : '', marginLeft: isPhone ? '1px' : '10%'}}/>
                  ) : (
                    <img src={useravatar} className="clientIcon fixed-size-image text-white text-2xl mr-4 cursor-pointer"  style={{width: isPhone ? '100%' : '100%', height: isPhone ? '120px' : '', marginLeft: isPhone ? '1px' : '10%'}} alt="Default User Icon"/>
                  )}
                  <br/>
                  <p className="clientname mt-[-10px] font-semibold text-black" style={{fontSize: isPhone ? '15px' : 'auto', marginTop: isPhone ? '-10px' : ''}}>{`${accounts.first_name || ''} ${accounts.middle_name ? accounts.middle_name + ' ' : ''}${accounts.last_name || ''}`}</p>
                </div>
              </div>
            </Link>
          ))}
